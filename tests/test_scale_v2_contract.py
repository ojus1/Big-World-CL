...
class RegistrationTests(unittest.TestCase):
    ...
    def test_coherently_rewritten_policy_is_not_the_reviewed_registration(self):
        m = deepcopy(self.manifest); p = m['launch_policy']
        p['workers'] = 1; p['per_world_wall_seconds'] = 3600; m['budgets'] = contract.budgets(p)
        for slot in m['slots']:
            slot['config'] = contract.config(slot['seed'], slot['algorithm'], p).public()
            slot['config_sha256'] = digest(slot['config'])
    ...
    def test_coherent_rehash_does_not_allow_shorter_horizon_or_method_change(self):
        ...
        for key, value in (('days', 8), ('train_cases', 1), ('hermes_transport', 'streaming'),
                           ('max_iterations', True), ('mirofish_service_url', 'http://127.0.0.1:5003')):
            def mutate(m):
