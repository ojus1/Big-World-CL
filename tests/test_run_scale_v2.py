...
from scripts import run_scale_v2 as launch
...
class SupervisorTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest = {'source_sha256': {}, 'registration_tools_sha256': {}, 'dependencies': {},
            'launch_policy': {'workers': 2, 'per_world_wall_seconds': 10,
        ...
        from scripts import audit_scale_v2
        self.audit = mock(audit_scale_v2, 'audit_campaign_v2', return_value={'ok': True, 'planned_runs': 6})
    ...
    def execute(self):
        return launch.execute(self.out, campaign_sha256='a' * 64, prerequisite_paths={})
    ...
    def test_six_slots_canonical_order_cleanup_and_exact_receipt_times(self):
        ...
        self.audit.assert_called_once_with(self.out, campaign_sha256='a' * 64, strict=True)
    ...
    def test_observation_failure_cleans_failed_world_and_continues_other_slots(self):
        ...
        rows = launch.contract.read(self.out / 'execution_results.json')['runs']
        self.assertEqual(len(rows), 6); self.assertEqual(rows[0]['termination_reason'], 'observation_failed')
