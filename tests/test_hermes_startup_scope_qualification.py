...
from scripts import hermes_startup_scope_qualification as q
...
def fixture(directory):
    ...
    m.update(schema_version=2,kind=q.VERSION,config=deepcopy(q.CONFIG),source_sha256=q.sources(),caller_security_context=LABEL)
    ...
    for slot in m['slots']:
        ...
        start=q.load(trial/'native/STARTUP_INTENT.json')[0];start.update(kind=q.VERSION,helper_sha256=m['source_sha256']['scripts/hermes_startup_scope_qualification.py']);write(trial/'native/STARTUP_INTENT.json',start)
    ...
    write(directory/'REPORT.json',{'schema_version':2,'kind':q.VERSION,'manifest_sha256':digest,
