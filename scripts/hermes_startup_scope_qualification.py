#!/usr/bin/env python3
"""Scope qualification v2: caller security context, fixed startup-only slots.
...
VERSION='hermes-startup-scope-qualification-v2'
BATCHES=(1,1,1,2,2,2)
CONFIG={**legacy.CONFIG,'ownership':'caller_exec_scope_with_held_controller',
        ...
        'invocation_environment_authority':'parent_verified_scope_readback'}
...
BOOTSTRAP=legacy.BOOTSTRAP.replace('scripts.hermes_startup_qualification import _worker',
                                  'scripts.hermes_startup_scope_qualification import _worker')
...
def sources():
    return {**legacy.sources(),'scripts/hermes_startup_scope_qualification.py':sha(Path(__file__).read_bytes())}
...
def prepare(out):
    ...
    manifest={'schema_version':2,'kind':VERSION,'config':CONFIG,'slots':slots,
...
def validate_manifest(directory,expected_hash,*,current=True):
    ...
    require(manifest['schema_version']==2 and manifest['kind']==VERSION
...
def _worker(directory,slot_id):
    ...
    require({k:os.environ.get(k) for k in ENV_KEYS}==manifest['controller_environment']
        and set(os.environ)<=set(ENV_KEYS),'controller_environment_not_scrubbed')
    # Scopes do not inject this service environment variable. Its authority is
    # the independently read, live scope InvocationID in the private parent gate.
    os.environ['INVOCATION_ID']=gate['unit_binding']['invocation_id']
...
def execute(out,*,manifest_sha256):
    ...
    save(directory/'EXECUTION.json',{'schema_version':2,'registered_manifest_sha256':manifest_sha256,
    ...
    save(directory/'REPORT.json',{'schema_version':2,'kind':VERSION,'manifest_sha256':manifest_sha256,
...
def _completed_evidence(trial, directory, manifest, execution, slot, intent, receipt):
    ...
    require(start_intent['kind'] == VERSION and start_intent['execution'] == child.EXECUTION
            and start_intent['helper_sha256'] == manifest['source_sha256']['scripts/hermes_startup_scope_qualification.py']
