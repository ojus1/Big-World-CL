...
def prepare(out, *, launch_policy, target_model, model_base_url, cohort_importer=import_cohort,
    ...
    contract.require(all(dependencies.get(name, {}).get('revision') for name in ('hermes', 'mirofish', 'skillopt')),
                     'v2_requires_installed_dependency_provenance')
    out = Path(out).resolve()
    contract.require(out.is_relative_to(ROOT / 'lifespan/artifacts'), 'v2_output_must_be_private_artifact_directory')
    ...
    for seed in contract.SEEDS:
        ...
        contract.require(len(ids) == len(set(ids)) == 25 and not seen.intersection(ids), 'v2_distinct_persona_cohorts')
        seen.update(ids)
    ...
    manifest = {'schema_version': 2, 'kind': contract.VERSION, 'created_at': datetime.now(timezone.utc).isoformat(),
